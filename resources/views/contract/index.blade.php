...
@section('action-button')
    <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-12 pt-lg-3 pt-xl-2">
        <div class="all-button-box">
            <a href="{{ route('contract.create',0) }}" class="btn btn-xs btn-white btn-icon-only width-auto">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body py-0">
                    ...
                    @else
                        {{ Form::open(['route' => ['customer.proposal'], 'method' => 'GET', 'id' => 'frm_submit']) }}
                    @endif
                        ...
                        <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12">
                            <div class="all-select-box">
                                <div class="btn-box">
                                    ...
                                    {{ Form::text('issue_date', isset($_GET['issue_date']) ? $_GET['issue_date'] : null, ['class' => 'form-control datepicker-range month-btn']) }}
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12">
                                ...
                                {{-- @foreach ($proposals as $proposal)
                                    <tr class="font-style">
                                        <td class="Id">
                                            @if (\Auth::guard('customer')->check())
                                                <a
                                                    href="{{ route('customer.proposal.show', \Crypt::encrypt($proposal->id)) }}">{{ AUth::user()->proposalNumberFormat($proposal->proposal_id) }}
                                                </a>
                                            @else
                                        ...
                                        <td>
                                            @if ($proposal->status == 0)
                                                <span
                                                    class="badge badge-pill badge-primary">{{ __(\App\Proposal::$statues[$proposal->status]) }}</span>
                                            @elseif($proposal->status == 1)
                                                <span
                                                    class="badge badge-pill badge-info">{{ __(\App\Proposal::$statues[$proposal->status]) }}</span>
                                            @elseif($proposal->status == 2)
                                                <span
                                                    class="badge badge-pill badge-success">{{ __(\App\Proposal::$statues[$proposal->status]) }}</span>
                                            @elseif($proposal->status == 3)
                                                <span
                                                    class="badge badge-pill badge-warning">{{ __(\App\Proposal::$statues[$proposal->status]) }}</span>
                                            @elseif($proposal->status == 4)
                                                <span
                                                    class="badge badge-pill badge-danger">{{ __(\App\Proposal::$statues[$proposal->status]) }}</span>
                                            @endif
                                        </td>
                                        @if (Gate::check('edit proposal') || Gate::check('delete proposal') || Gate::check('show proposal'))
                                            <td class="Action">
                                ...
                                {{-- @foreach ($contracts as $contract)
                                    <tr class="font-style">
                                        <td class="Id">
                                            @if (\Auth::guard('customer')->check())
                                                <a
                                                    href="{{ route('customer.contract.show', \Crypt::encrypt($contract->id)) }}">{{ AUth::user()->contractNumberFormat($contract->contract_id) }}
                                                </a>
                                            @else
                                                <a href="{{ route('contract.show', \Crypt::encrypt($contract->id)) }}">{{ AUth::user()->contractNumberFormat($contract->contract_id) }}
                                                </a>
                                            @endif
                                        </td>
                                        @if (!\Auth::guard('customer')->check())
                                        ...
                                        @endif
                                        <td>{{ !empty($contract->category) ? $contract->category->name : '' }}</td>
                                        <td>{{ Auth::user()->dateFormat($contract->issue_date) }}</td>
                                        <td>
                                            @if ($contract->status == 0)
                                                <span
                                                    class="badge badge-pill badge-primary">{{ __(\App\contract::$statues[$contract->status]) }}</span>
                                            @elseif($contract->status == 1)
                                                <span
                                                    class="badge badge-pill badge-info">{{ __(\App\contract::$statues[$contract->status]) }}</span>
                                            @elseif($contract->status == 2)
                                                <span
                                                    class="badge badge-pill badge-success">{{ __(\App\contract::$statues[$contract->status]) }}</span>
                                            @elseif($contract->status == 3)
                                                <span
                                                    class="badge badge-pill badge-warning">{{ __(\App\contract::$statues[$contract->status]) }}</span>
                                            @elseif($contract->status == 4)
                                                <span
                                                    class="badge badge-pill badge-danger">{{ __(\App\contract::$statues[$contract->status]) }}</span>
                                            @endif
                                        </td>
                                        @if (Gate::check('edit contract') || Gate::check('delete contract') || Gate::check('show contract'))
                                            <td class="Action">
                                                @if ($contract->is_convert == 0)
                                                    @can('convert invoice')
                                                        <a href="#" class="edit-icon bg-warning" data-toggle="tooltip"
                                                            data-original-title="{{ __('Convert to Invoice') }}"
                                                            data-toggle="tooltip" data-original-title="{{ __('Delete') }}"
                                                            data-confirm="{{ __('You want to confirm convert to invoice. Press Yes to continue or Cancel to go back') }}"
                                                            data-confirm-yes="document.getElementById('contract-form-{{ $contract->id }}').submit();">
                                                            <i class="fas fa-exchange-alt"></i>
                                                            {!! Form::open(['method' => 'get', 'route' => ['contract.convert', $contract->id], 'id' => 'contract-form-' . $contract->id]) !!}
                                                            {!! Form::close() !!}
                                                        </a>
                                                    @endcan
                                                @else
                                                    @can('convert invoice')
                                                        <a href="{{ route('invoice.show', \Crypt::encrypt($contract->converted_invoice_id)) }}"
                                                            class="edit-icon bg-warning" data-toggle="tooltip"
                                                            data-original-title="{{ __('Already convert to Invoice') }}"
                                                            data-toggle="tooltip" data-original-title="{{ __('Delete') }}">
                                                            <i class="fas fa-file"></i>
                                                    ...
                                                    @endcan
                                                @endif
                                                @can('duplicate contract')
                                                    <a href="#" class="edit-icon bg-success" data-toggle="tooltip"
                                                        data-original-title="{{ __('Duplicate') }}" data-toggle="tooltip"
                                                        data-original-title="{{ __('Delete') }}"
                                                        data-confirm="{{ __('You want to confirm duplicate this invoice. Press Yes to continue or Cancel to go back') }}"
                                                        data-confirm-yes="document.getElementById('duplicate-form-{{ $contract->id }}').submit();">
                                                        <i class="fas fa-copy"></i>
                                                        {!! Form::open(['method' => 'get', 'route' => ['contract.duplicate', $contract->id], 'id' => 'duplicate-form-' . $contract->id]) !!}
                                                        {!! Form::close() !!}
                                                    </a>
                                                @endcan
                                                @can('show contract')
                                                    @if (\Auth::guard('customer')->check())
                                                        <a href="{{ route('customer.contract.show', \Crypt::encrypt($contract->id)) }}"
                                                            class="edit-icon bg-info" data-toggle="tooltip"
                                                            data-original-title="{{ __('Detail') }}">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                ...
                                                @endcan
                                                @can('edit contract')
                                                    <a href="{{ route('contract.edit', \Crypt::encrypt($contract->id)) }}"
                                                        class="edit-icon" data-toggle="tooltip"
                                                        data-original-title="{{ __('Edit') }}">
                                                        <i class="fas fa-pencil-alt"></i>
                                                    </a>
